...
└─────────────────────┬───────────────────────────────────────────────┘
                      ...
                      ▼
┌─────────────────────────────────────────────────────────────────────┐
│              GLOBAL REMEDIATOR  (AI SOAR Agent · HITL)               │
│  Plans remediation for every CRITICAL / HIGH / MEDIUM alert and      │
│  routes it to a human analyst for Approve / Reject / Defer.          │
│  NEVER executes autonomously. LOW / INFO auto-closed.                │
└─────────────────────┬───────────────────────────────────────────────┘
                      │
                      ▼
┌─────────────────────────────────────────────────────────────────────┐
│                         DATA PERSISTENCE LAYER                       │
│       PostgreSQL (alerts, scores, snapshots, remediation log)        │
...
┌─────────────────────────────────────────────────────────────────────┐
│                        PRESENTATION LAYER                            │
│   Executive Dashboard │ Analyst Dashboard │ Remediator Queue │ API   │
│                    React + Chart.js + D3                             │
└─────────────────────────────────────────────────────────────────────┘
```
...
### 7.5 Global Remediator (Human-in-the-Loop SOAR)
- AI-generated remediation plan for every CRITICAL, HIGH and MEDIUM alert
- Interactive approval queue: analyst Approves, Rejects, or Defers each plan
- Per-action impact assessment and AI confidence score
- Tamper-evident decision audit log (who decided what, when)
- Agent operating policy enforced: no autonomous execution; LOW/INFO auto-closed
- Works identically across all five industry verticals
...
Vendor API → Connector → CAS Normaliser → Risk Engine → DB → Dashboard
            ...
            ┌──────────────────────────────────────────┐
            │  GLOBAL REMEDIATOR plans remediation      │
            │  → Approval Queue → Human decision        │
            │  Approve → dispatch to control's API      │
            │  Reject  → return to SOC queue            │
            │  Defer   → re-prompt after snooze window  │
            │  (LOW / INFO → auto-close, no prompt)     │
            └──────────────────────────────────────────┘
```
