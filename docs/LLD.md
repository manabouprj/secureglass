...
### 4.4 Global Remediator (Human-in-the-Loop SOAR Agent)

The Global Remediator is a distinct agent that sits downstream of the Risk Scorer. For every
alert scored CRITICAL, HIGH, or MEDIUM it generates a structured remediation plan and places it
in an approval queue. **It never executes an action autonomously** — a human analyst must Approve,
Reject, or Defer. LOW and INFO alerts are auto-closed with no human prompt.

#### Remediation object schema

```json
{
  "remediation_id": "uuid-v4",
  "cas_id": "uuid (FK to cas_alerts)",
  "vertical": "enterprise|healthcare|financial|energy|retail",
  "severity": "CRITICAL|HIGH|MEDIUM",
  "target_tool": "crowdstrike|wiz|qualys|...",
  "affected_asset": "hostname / account / resource",
  "proposed_steps": ["ordered list of remediation actions"],
  "impact_assessment": "plain-English blast-radius + disruption estimate",
  "ai_confidence": 0.0,
  "requires_cosign": false,
  "status": "PENDING|APPROVED|REJECTED|DEFERRED",
  "decided_by": "analyst id (null until decided)",
...
```
  ...
  │ dispatch │  │ → SOC    │  │ snooze 4h│
  │ to tool  │  │   queue  │  │ re-prompt│
  └──────────┘  └──────────┘  └──────────┘
```

#### Operating policy (enforced)

| Severity | Action |
|----------|--------|
| CRITICAL | Generate plan, page on-call, **block auto-exec**, require approval (co-sign for OT safety-critical) |
| HIGH | Generate plan, queue for analyst approval |
| MEDIUM | Generate plan, queue for analyst approval |
| LOW / INFO | Auto-close, no human prompt, log only |
...
CREATE INDEX idx_rem_status ON remediations(status);
CREATE INDEX idx_rem_cas ON remediations(cas_id);
```

#### Claude prompt (plan generation)

```python
def generate_remediation_plan(alert: dict) -> dict:
    prompt = f"""
    You are the Global Remediator, a SOAR agent for a {alert['vertical']} security team.
    A {alert['severity']} alert fired:
      Title: {alert['title']}
      Tool: {alert['source_tool']}
      Asset: {alert['affected_assets']}
      MITRE: {alert['mitre_technique']} ({alert['mitre_tactic']})

    Propose a remediation plan as JSON only:
      - proposed_steps: 3-5 concrete, ordered actions (tool-specific where possible)
      - impact_assessment: blast radius + business disruption in one sentence
      - ai_confidence: 0-100
      - requires_cosign: true if action could affect safety/availability
    Do NOT assume execution — a human will approve. Be conservative for
    safety-critical assets (OT/medical devices).
    """
    # Call Claude API; parse JSON; persist to remediations table with status=PENDING
    ...
```

#### Audit & compliance

Every decision writes an immutable row update (`decided_by`, `decided_at`, `status`) plus an
append-only entry to the decision log. This satisfies the "human accountability" control required
by SOC 2 CC7.4, ISO 27001 A.5.26, NIST CSF RS.MI, and DORA incident-response evidence.
...
# Global Remediator (Human-in-the-Loop)
GET  /api/v1/remediations?status=PENDING  → []Remediation (approval queue)
GET  /api/v1/remediations/:id             → Remediation
POST /api/v1/remediations/:id/approve      → 200 (dispatch to target tool, log decision)
POST /api/v1/remediations/:id/reject       → 200 (return to SOC queue, log decision)
POST /api/v1/remediations/:id/defer        → 200 (snooze, re-prompt after window)
GET  /api/v1/remediations/log              → []DecisionLogEntry (audit trail)
```
